...
describe('Sign up and Sign in testing', function(){
  ...
  it('Checks to see if password matches confirmPassword', function(done){
    request(app).post('/users/sign_up')
    .send(confirm_pass_wrong)
    .end(function(err, response){
  ...
  //checks to see if we get usetname already exists if we create an account which already uses that username
  it('Checks to see if username is valid', function(done){
    request(app).post('/users/sign_up')
    .send(username_exists)
    .end(function(err, response){
      expect(response.statusCode).to.equal(200);
